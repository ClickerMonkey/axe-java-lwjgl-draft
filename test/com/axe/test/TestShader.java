...
public class TestShader 
{
	...
	public static void main(String[] args)
	{
		...
		game.on( GameEvent.Start, g -> {
			Axe.logger.log( Axe.graphics.getVersion() );
	...
	private static class TestNode extends RenderedNode<Vec2f>
	{
		...
		public TestNode()
		{
			...
			this.shader = Axe.graphics.newShader(
				Assets.load("assets/shaders/default.vert", ShaderObject.class),
				Assets.load("assets/shaders/pixelize.frag", ShaderObject.class)
			);
		}
	...
	private static class TestNodeRenderer implements Renderer<TestNode>
	{
		...
		public void begin(TestNode entity, GameState state, View view) 
		{
			...
			shader.bind();
			shader.set( "texUnit", 0 );
			shader.set( "pixel", new Vec2f( 
				tex.getPixelWidth() * 2, 
				tex.getPixelHeight() * 2		
			));
